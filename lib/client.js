...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* The stock Tooltip (the one the message like/dislike buttons use):
		   takes a hover delay (`delayMs`) and flips side near the viewport
		   edge. Without it the chip falls back to a native
		   `title`, which the browser shows only after a hover pause. */
		const Tooltip = typeof primitives?.Tooltip === "function" ? primitives.Tooltip : null;
		...
		function EffortChip(props) {
			...
			/* The reason shows after a short hover (stock Tooltip, 400 ms — the
			   delay the stock agent-preset cards use): quicker than the native
			   title's pause, but not on every pass of the pointer across the
			   composer. It stands down while the popover is open, which states
			   the reason itself. */
			const chip = React.createElement("button", {
				type: "button",
				className: "dshjev-chip",
				title: Tooltip === null ? title : undefined,
				"aria-expanded": open,
				onClick: () => setOpen((v) => !v),
			}, ...body);
			return React.createElement("div", { ref: rootRef, className: "dshjev-chipRoot" },
				Tooltip === null ? chip : React.createElement(Tooltip, { label: title, side: "top", delayMs: 400, disabled: open }, chip),
		...
		function LevelsSection(props) {
			...
			return React.createElement("div", { style: cardCSS.field },
				...
				/* One explanation for the whole list, shown open or closed, instead
				   ...
				   their selection is invalid. Clicking it toggles the section
				   too, like the heading above it; the heading stays the one
				   control keyboards and screen readers use. */
				React.createElement("p", { style: { ...cardCSS.hint, cursor: "pointer" }, onClick: toggleOpen }, "为每个模型指定 Jev 可选的推理档位。自动：高亮的是插件推导出的档位；自定义：Jev 只会在你选中的档位里挑选。"),
				body);
		}
