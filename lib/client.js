...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* The stock Tooltip (the one the message like/dislike buttons use):
		   shows on pointer enter with no delay by default and flips side near
		   the viewport edge. Without it the chip falls back to a native
		...
		function EffortChip(props) {
			...
			/* The reason shows the moment the pointer lands on the chip (stock
			   Tooltip, no delay), not after the native title's hover pause. It
			   stands down while the popover is open, which states the reason
			   itself. */
			...
			return React.createElement("div", { ref: rootRef, className: "dshjev-chipRoot" },
				Tooltip === null ? chip : React.createElement(Tooltip, { label: title, side: "top", disabled: open }, chip),
