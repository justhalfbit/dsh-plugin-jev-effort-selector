	...
	factory: (require) => {
		...
		const CSS = [
			...
			/* Not decided by Jev this turn (a failure), or Jev off for this
			   session: the whole chip drops to caption grey (the stock effort
			   label's colour). label-dimmed was too faint to read on white. */
			".dshjev-chip.dshjev-chipMuted,.dshjev-chipMuted .dshjev-effort.dshjev-effort{color:var(--dsw-alias-label-caption)}",
		].join("");
		 ...
		 *
		 * A number field's `min` / `max` mirror the host schema (`settingsFields`
		 * in lib/index.js) — keep the two in step. The form checks them before
		 * Save because DSH 0.1.5 refuses an out-of-range write without telling
		 * the form: the box would silently snap back to the old value.
		 */
		const FIELDS = [
			...
			{ field: "confidenceThreshold", kind: "number", min: 0, max: 1, label: "置信度阈值", hint: "低于该值时，在概率最高的两档中选更高的那个。" },
			{ field: "timeoutMs", kind: "number", min: 500, max: 30000, label: "超时时间（毫秒）", hint: "超时后跳过 Jev，沿用调用方已解析出的等级。" },
		...
		class JevForm {
			...
			fieldState(field) {
				...
				// An emptied box is valid: Save turns it into "back to the default".
				const trimmed = staged.text.trim();
				const error = spec.kind === "number" && trimmed !== "" ? numberError(spec, trimmed) : "";
